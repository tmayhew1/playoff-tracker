  ...
  return (
    <div className="space-y-5">
      ...
      <Step n="9" title="Legacy">
        ...
        <p className="mb-2">Careers get <span className="font-semibold">two numbers, not one</span>. <span className="font-semibold">Legacy</span> sorts a player&apos;s seasons best-first and discounts each one below the last, so extra years always add — just less each time, and hanging on for a few replacement-level seasons is neither rewarded nor punished. <span className="font-semibold">Peak/G</span> is the same leverage weighting as a rate over the best seven seasons. They disagree, and the disagreement is the point: tap either column to sort by it. The decay behind that fold isn&apos;t a taste — it&apos;s calibrated against the corpus to the point where the board sits exactly between a plain career sum and a best-season ranking.</p>
        <p>Tap a player to open the fold itself — every season in the order the decay is spent, with the discount it took and the discounted value that sums to the total. The bar under each season splits it: dark is the playoff run, light the regular season. A handful of games usually outweighs a whole winter, which is leverage doing its job.</p>
      </Step>
