  ...
  return (
    <div className="space-y-5">
      ...
      <Step n="9" title="Legacy">
        ...
        <p className="mb-2">The two teams split that stake by the <span className="font-semibold">margin</span> the series was decided by, squared — so the eight ways a series can end fall in order, sweeping it worth the most and being swept the least, with the smallest gap of all between winning in seven and losing in seven. A series that goes the distance says the two teams were even, and it is priced as the coin flip it was; a sweep is not.</p>
        <p className="mb-2">Careers get <span className="font-semibold">two numbers, not one</span>. <span className="font-semibold">Legacy</span> is a career&apos;s leveraged seasons added up — every one at face value, nothing discounting the lesser years against the best, and a season below the league&apos;s typical minute subtracting what it cost. That makes it volume, and says so: an extra good-not-great year always helps. <span className="font-semibold">Peak/G</span> is the same leverage weighting as a rate over the best seven seasons, and it is the answer to that. They disagree, and the disagreement is the point: tap either column to sort by it.</p>
        <p>Tap a player to open the career up — every season in value order, summing to the total. The bar under each season splits it: dark is the playoff run, light the regular season. A handful of games usually outweighs a whole winter, which is leverage doing its job.</p>
      </Step>
